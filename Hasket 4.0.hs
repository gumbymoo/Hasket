...
eventLoop env = do exp <- getLine
                   if exp == "showEnv" then error $ show env else do
...
applyFunction2 env (Right (func,_)) args = let evaledArgs = map (eval . (\x -> (x,env))) args
                                               funcPieces = getExps $ chopParentheses func
                                               parameters = words $ chopParentheses $ funcPieces !! 2
                                               funcBody = tail $ foldl (\x y -> x ++ " " ++ y) [] (drop 3 funcPieces)
                                               funcEnvironment = (read $ funcPieces !! 1) -- :: Environment
                                               in concatEvaledArgs evaledArgs >>= (\y -> (eval (funcBody,(zip parameters y):(funcEnvironment ++ env)))) >>= (\(r,(x:xs)) -> Right (r,xs))


makeFunction :: String -> Environment -> String
makeFunction str env
  | take 7 str /= "(lambda" = error "Bad function definition -- if you're seeing me, you goofed"
